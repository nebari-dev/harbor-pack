...
test('is idempotent (does not double-prefix)', () => {
  const [a] = run([el('a', { href: '/harbor-pack/auth-flow/' })]);
  expect(a.properties.href).toBe('/harbor-pack/auth-flow/');
});
